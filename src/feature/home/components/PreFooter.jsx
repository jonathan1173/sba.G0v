import React, { useState } from "react"; // 1. Importar useState

// --- Datos para las columnas de enlaces (sin cambios) ---
const aboutSbaLinks = [
  { text: "Contact SBA", href: "#" },
  { text: "Locations", href: "#" },
  { text: "Upcoming events", href: "#" },
  { text: "Newsroom", href: "#" },
  { text: "SBA blog", href: "#" },
  { text: "Leadership team", href: "#" },
];
const aboutSiteLinks = [
  { text: "Site map", href: "#" },
  { text: "Privacy policy", href: "#" },
  { text: "Linking policy", href: "#" },
  { text: "Plain language", href: "#" },
  { text: "Accessibility", href: "#" },
  { text: "Disclaimers", href: "#" },
];
const openGovLinks = [
  { text: "Freedom of Information Act", href: "#" },
  { text: "No Fear Act", href: "#" },
  { text: "Open data sources", href: "#" },
  { text: "Policies and guidance", href: "#" },
  { text: "Budget and performance", href: "#" },
  { text: "Regulations.gov", href: "#", external: true },
];
const oversightLinks = [
  { text: "Inspector General", href: "#" },
  { text: "Advocacy", href: "#" },
  { text: "Hearings and appeals", href: "#" },
  { text: "Ombudsman", href: "#" },
  { text: "Fraud and identity theft", href: "#" },
  { text: "WhiteHouse.gov", href: "#", external: true },
  { text: "USA.gov", href: "#", external: true },
];
const linkColumns = [
  { title: "About SBA", links: aboutSbaLinks },
  { title: "About the site", links: aboutSiteLinks },
  { title: "Open government", links: openGovLinks },
  { title: "Oversight", links: oversightLinks },
];

// --- El Componente del Footer ---
function Footer() {
  // 2. Añadir estado para manejar el acordeón
  const [openAccordion, setOpenAccordion] = useState(null);

  const toggleAccordion = (title) => {
    setOpenAccordion(openAccordion === title ? null : title);
  };

  // --- Contenido del Formulario (separado para reutilizarlo) ---
  // Se usa el mismo formulario para móvil y escritorio para evitar duplicar código
  const SignUpForm = ({ formId }) => (
    <>
      <h2 className="text-2xl font-bold text-gray-900 mb-3">Sign up</h2>
      <p className="text-sm text-gray-700 mb-2">
        Receive information about upcoming SBA events, news alerts, and program
        updates.
      </p>
      <p className="text-sm mb-4">
        <span className="text-red-600">*</span> indicates a required field
      </p>
      <form action="#">
        <div className="mb-4">
          <label
            htmlFor={`email-${formId}`} // IDs únicos
            className="block text-sm font-medium text-gray-700 mb-1"
          >
            Email <span className="text-red-600">*</span>
          </label>
          <input
            type="email"
            id={`email-${formId}`}
            className="w-full p-2 border border-gray-400 rounded-md shadow-sm"
          />
        </div>
        <div className="mb-4">
          <label
            htmlFor={`zip-${formId}`}
            className="block text-sm font-medium text-gray-700 mb-1"
          >
            ZIP Code <span className="text-red-600">*</span>
          </label>
          <input
            type="text"
            id={`zip-${formId}`}
            className="w-full p-2 border border-gray-400 rounded-md shadow-sm"
          />
        </div>
        <button
          type="submit"
          className="w-full bg-blue-600 text-white font-medium py-2 px-4 rounded-md hover:bg-blue-700 transition-colors"
        >
          Subscribe to SBA emails
        </button>
      </form>
      <p className="text-xs text-gray-600 mt-4">
        Your information will only be used in accordance with our
        <a href="#" className="text-blue-700 hover:underline ml-1">
          website privacy policy
        </a>
        .
      </p>
    </>
  );

  return (
    // 3. Quitar h-[100vh] y dejar solo el padding
    <footer className="bg-gray-100 text-gray-800  md:p-12 font-sans">
      <div className="max-w-7xl mx-auto">
        {/*
        ============================================================
        |                    VISTA DE ESCRITORIO                   |
        | (Se oculta en móvil, se muestra como grid en escritorio) |
        ============================================================
        */}
        <div className="hidden md:grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-8">
          {/* --- Columnas de Enlaces (Escritorio) --- */}
          {linkColumns.map((column) => (
            <div key={column.title}>
              <h3 className="font-semibold text-gray-900 mb-4">
                {column.title}
              </h3>
              <ul className="space-y-2">
                {column.links.map((link) => (
                  <li key={link.text}>
                    <a
                      href={link.href}
                      className="text-blue-700 underline text-[18px]"
                    >
                      {link.text}
                      {link.external && (
                        <span className="ml-1 text-xs">&#8599;</span>
                      )}
                    </a>
                  </li>
                ))}
              </ul>
            </div>
          ))}

          {/* --- Columna del Formulario (Escritorio) --- */}
          <div className="md:col-span-2 lg:col-span-2 p-8">
            <SignUpForm formId="desktop" />
          </div>
        </div>

        {/*
        ========================================================
        |                     VISTA DE MÓVIL                   |
        | (Se muestra en móvil, se oculta en escritorio)       |
        ========================================================
        */}
        <div className="block md:hidden">
          {/* --- Acordeón de Enlaces (Móvil) --- */}
          <div className="border-t border-gray-300">
            {linkColumns.map((column) => (
              <div key={column.title} className="border-b border-gray-300">
                <button
                  onClick={() => toggleAccordion(column.title)}
                  className="flex justify-between items-center w-full p-4 text-left"
                >
                  <span className="text-lg font-semibold text-gray-900">
                    {column.title}
                  </span>
                  
                  {/* 4. CAMBIO: Iconos SVG eliminados, reemplazados por texto */}
                  <span className="font-bold text-lg">
                    {openAccordion === column.title ? "v" : ">"}
                  </span>
                  
                </button>

                {/* --- Contenido del acordeón --- */}
                {openAccordion === column.title && (
                  <div className="p-4 bg-white">
                    <ul className="space-y-3">
                      {column.links.map((link) => (
                        <li key={link.text}>
                          <a
                            href={link.href}
                            className="text-blue-800 underline text-base"
                          >
                            {link.text}
                            {link.external && (
                              <span className="ml-1 text-xs">&#8599;</span>
                            )}
                          </a>
                        </li>
                      ))}
                    </ul>
                  </div>
                )}
              </div>
            ))}
          </div>

          {/* --- Formulario "Sign up" (Móvil) --- */}
          <div className="mt-8 p-8">
            <SignUpForm formId="mobile" />
          </div>
        </div>
      </div>
    </footer>
  );
}

export default Footer;

